...
class AppTest(unittest.TestCase):
    ...
    def test_if_get_link(self):
        self.assertTrue(hasattr(FeedService, 'get_link'))

    def test_if_get_title(self):
        self.assertTrue(hasattr(FeedService, 'get_title'))

    def test_if_generate_text(self):
        self.assertTrue(hasattr(FeedService, 'generate_text'))

    def test_generate_text(self):
        self.assertEqual(FeedService.generate_text(['Hello', 'World']), ' Hello World')
        self.assertEqual(FeedService.generate_text(['Todavia,', 'o', 'início', 'da', 'atividade', 'geral']),
                         ' Todavia, o início da atividade geral')

    def test_get_link(self):
        self.assertEqual(
            FeedService.get_link('<link>Hello World</link><link>No Hello World</link>', 0), 'Hello World')
        self.assertEqual(
            FeedService.get_link('<link>Hello World</link><link>No Hello World</link>', 1), 'No Hello World')
        self.assertNotEqual(
            FeedService.get_link('<link>Hello World</link><link>No Hello World</link>', 0), 'No Hello World')

    def test_get_title(self):
        self.assertEqual(
            FeedService.get_title('<title><![CDATA[Hello World]]></title><title><![CDATA[No Hello World]]></title>', 0),
            'Hello World')
        self.assertEqual(
            FeedService.get_title('<title><![CDATA[Hello World]]></title><title><![CDATA[No Hello World]]></title>', 1),
            'No Hello World')
        self.assertNotEqual(
            FeedService.get_title('<title><![CDATA[Hello World]]></title><title><![CDATA[No Hello World]]></title>', 0),
            'No Hello World')
    ...
    def test_xml_to_json(self):
        ...
        self.assertEqual(json.dumps(FeedService.xml_to_json(xml), ensure_ascii=False), feed)
